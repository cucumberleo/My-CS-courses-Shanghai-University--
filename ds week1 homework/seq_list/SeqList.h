...
// 顺序表的函数成员
	...
	void RemoveRep();//删除重复元素 homework 1(3)
	void RemoveStT(int s,int t);//homework
	void Reverse();
	void print() const;
};
...
template<class T>
void SeqList<T>::RemoveRep() {
    unordered_set<T> vis;  
    int idx = 0;
    for (int i = 0; i < length; i++) {
        if (vis.find(elems[i]) == vis.end()) {
            elems[idx++] = elems[i];
            vis.insert(elems[i]);
        }
    } 
...
template<class T>
void SeqList<T>::RemoveStT(int s,int t){
	if(s>=t) throw std::runtime_error("请输入正确的s和t");
	for(int i=0;i<length;){
		if(elems[i]>=s && elems[i]<=t) DeleteElem(i,elems[i]);
		else ++i;
	}
}
template<class T>
